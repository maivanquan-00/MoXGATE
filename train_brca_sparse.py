"""
train_brca_sparse.py
====================
Training script cho MoXGATE với SPARSEMAX modality fusion — dành riêng cho BRCA.

Mục tiêu: Thử nghiệm xem việc cho phép gán trọng số modality bằng 0 (Sparsemax)
có giúp cải thiện độ chính xác phân loại so với Softmax truyền thống không.
"""
...
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    ...
    try:
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges', # Dùng màu cam để phân biệt với bản softmax (Blue)
                    xticklabels=labels, yticklabels=labels)
        plt.title('Confusion Matrix - BRCA Subtypes (Sparsemax)')
...
def train(args):
    ...
    print(f"\n{'★'*60}")
    print("  MoXGATE — TRAINING (BRCA - SPARSEMAX EXPERIMENT)")
    print(f"{'★'*60}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict(), "config": model.config}, 
                       os.path.join(args.save_dir, "best_model_brca_sparse.pt"))
        else:
            patience_counter += 1
            if patience_counter >= args.patience: break

    # ── Evaluation ──
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_brca_sparse.pt"), map_location=device)
    ...
    print(f"\n[Sparsemax Result] Accuracy: {test_metrics['accuracy']:.4f} | F1: {test_metrics['weighted_f1']:.4f}")
    print(f"Classification Report:\n{classification_report(test_targets, test_preds, labels=present_labels, target_names=present_names, zero_division=0)}")
    
    cm_path = os.path.join(args.save_dir, "confusion_matrix_brca_sparse.png")
    plot_confusion_matrix(test_targets, test_preds, labels=present_names, save_path=cm_path)

    with open(os.path.join(args.save_dir, "history_brca_sparse.json"), "w") as f: json.dump(history, f, indent=2)
...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str, default=config.BRCA_FINAL_DIR)
    parser.add_argument("--save_dir", type=str, default=config.BRCA_CHECKPOINT_DIR)
