...
train_kipan.py
==============
Training script cho MoXGATE (SOFTMAX) — KIPAN (Kidney Cancer).

3 phân lớp: KICH (0) / KIRC (1) / KIRP (2)
Tổng: ~688 mẫu | Split: 80/10/10 Stratified

Cách chạy:
    !python MoXGATE/train_kipan.py
    !python MoXGATE/train_kipan.py --data_dir /path/to/data_final_kipan
"""
...
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    try:
        ...
        plt.title('Confusion Matrix - Kidney Cancer KIPAN (Softmax)')
...
def train(args):
    ...
    print(f"\n{'★'*60}")
    print("  MoXGATE — TRAINING (KIPAN Kidney Cancer — Softmax)")
    print(f"{'★'*60}")
    print(f"  Device     : {device}")
    print(f"  Data dir   : {args.data_dir}")
    print(f"  Save dir   : {args.save_dir}")
    print(f"  Classes    : KICH(0) / KIRC(1) / KIRP(2)")
    print(f"{'★'*60}\n")
    ...
    print(f"[Train KIPAN] Trainable parameters: {model.count_parameters():,}\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict(), "config": model.config,
                        "dims": dims, "dataset": "KIPAN"},
                       os.path.join(args.save_dir, "best_model_kipan.pt"))
            print(f"  ✓ Saved best model (val_acc={best_val_acc:.4f})")
        else:
            patience_counter += 1
            if patience_counter >= args.patience:
                print(f"\n[Train KIPAN] Early stopping tại epoch {epoch}")
                break

    # ── Test Evaluation ──
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_kipan.pt"), map_location=device)
    ...
    print(f"\n{'─'*60}")
    print("  ĐÁNH GIÁ TRÊN TEST SET (KIPAN — Stratified 20%)")
    print(f"{'─'*60}")
    ...
    cm_path = os.path.join(args.save_dir, "confusion_matrix_kipan.png")
    plot_confusion_matrix(test_targets, test_preds, labels=present_names, save_path=cm_path)

    with open(os.path.join(args.save_dir, "history_kipan.json"), "w") as f:
        json.dump(history, f, indent=2)
    with open(os.path.join(args.save_dir, "test_results_kipan.json"), "w") as f:
        json.dump({"test_metrics": test_metrics, "modality_weights": model.get_modality_weights(),
                   "dataset": "KIPAN", "subtype_names": KIPAN_SUBTYPE_NAMES}, f, indent=2)

    print(f"\n{'★'*60}")
    print(f"  HOÀN TẤT KIPAN — Val Acc: {best_val_acc:.4f} | Test Acc: {test_metrics['accuracy']:.4f} | Test F1: {test_metrics['weighted_f1']:.4f}")
    print(f"{'★'*60}")
    return test_metrics
