...
train_new.py
============
Training script cho MoXGATE (SOFTMAX) — Sử dụng chiến lược chia dữ liệu STRATIFIED mới cho GI.
"""
...
# ... (utils, evaluate function same as original)
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    try:
        ...
        plt.title('Confusion Matrix - GI New Split (Softmax)')
...
def train(args):
    ...
    print(f"\n[Softmax New Split] Starting training on GI dataset...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict(), "config": model.config}, 
                       os.path.join(args.save_dir, "best_model_gi_new.pt"))
        else:
            patience_counter += 1
            if patience_counter >= args.patience: break

    # Final Test
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_gi_new.pt"), map_location=device)
    ...
    print(f"\n[Softmax New Split Result] Accuracy: {test_metrics['accuracy']:.4f} | F1: {test_metrics['weighted_f1']:.4f}")
    print(classification_report(test_targets, test_preds, target_names=present_names, zero_division=0))
    
    cm_path = os.path.join(args.save_dir, "confusion_matrix_gi_new.png")
