"""
train_sparse_balanced.py
========================
Training script cho MoXGATE (SPARSEMAX) — Sử dụng cấu hình CÂN BẰNG MẪU (Oversampling).
"""
...
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    try:
        ...
        plt.title('Confusion Matrix - GI Balanced (Sparsemax)')
...
def train(args):
    ...
    print(f"\n[Sparsemax Balanced] Starting training on GI dataset...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict()}, os.path.join(args.save_dir, "best_model_sparse_gi_balanced.pt"))
    ...
    # Evaluation
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_sparse_gi_balanced.pt"), map_location=device)
    ...
    print(f"\n[Sparsemax Balanced Result] Accuracy: {test_metrics['accuracy']:.4f} | F1: {test_metrics['f1']:.4f}")
    print(classification_report(test_targets, test_preds, target_names=present_names, zero_division=0))
    plot_confusion_matrix(test_targets, test_preds, present_names, os.path.join(args.save_dir, "confusion_matrix_sparse_gi_balanced.png"))
