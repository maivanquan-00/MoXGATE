...
train_ucec.py
==============
Training script cho MoXGATE (SOFTMAX) — UCEC (Uterine Corpus Endometrial Carcinoma).

4 phân lớp: CN_LOW (0), CN_HIGH (1), MSI (2), POLE (3)
Tổng: ~500 mẫu | Split: 80/10/10 Stratified

Cách chạy:
    !python MoXGATE/train_ucec.py
"""
...
import config
from dataset_ucec import build_dataloaders, UCEC_SUBTYPE_NAMES, NUM_CLASSES_UCEC
...
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    try:
        ...
        plt.title('Confusion Matrix - UCEC (Softmax)')
...
def train(args):
    ...
    print(f"\n{'★'*60}")
    print("  MoXGATE — TRAINING (UCEC — Softmax)")
    print(f"{'★'*60}")
    ...
    model = MoXGATE(
        ...
        num_classes=NUM_CLASSES_UCEC,
    ).to(device)
    model.set_class_weights(class_weights)

    print(f"[Train UCEC] Trainable parameters: {model.count_parameters():,}\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch:3d}/{args.epochs} | "
              ...
              f"Val Acc: {val_metrics['accuracy']:.4f} | Val W-F1: {val_metrics['weighted_f1']:.4f} | Val M-F1: {val_metrics['macro_f1']:.4f} | "
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict(), "config": model.config,
                        "dims": dims, "dataset": "UCEC"},
                       os.path.join(args.save_dir, "best_model_ucec.pt"))
            print(f"  ✓ Saved best model (val_acc={best_val_acc:.4f})")
        else:
            patience_counter += 1
            if patience_counter >= args.patience:
                print(f"\n[Train UCEC] Early stopping tại epoch {epoch}")
                break

    # ── Test Evaluation ──
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_ucec.pt"), map_location=device)
    ...
    present_names  = [UCEC_SUBTYPE_NAMES[i] for i in present_labels]

    print(f"\n{'─'*60}")
    print("  ĐÁNH GIÁ TRÊN TEST SET (UCEC)")
    print(f"{'─'*60}")
    ...
    cm_path = os.path.join(args.save_dir, "confusion_matrix_ucec.png")
    plot_confusion_matrix(test_targets, test_preds, labels=present_names, save_path=cm_path)

    with open(os.path.join(args.save_dir, "history_ucec.json"), "w") as f:
        json.dump(history, f, indent=2)
    with open(os.path.join(args.save_dir, "test_results_ucec.json"), "w") as f:
        json.dump({"test_metrics": test_metrics, "modality_weights": model.get_modality_weights(),
                   "dataset": "UCEC", "subtype_names": UCEC_SUBTYPE_NAMES}, f, indent=2)

    print(f"\n{'★'*60}")
    print(f"  HOÀN TẤT UCEC — Val Acc: {best_val_acc:.4f} | Test Acc: {test_metrics['accuracy']:.4f} | Test F1: {test_metrics['weighted_f1']:.4f}")
    print(f"{'★'*60}")
    return test_metrics
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MoXGATE — UCEC")
    parser.add_argument("--data_dir",     type=str,   default=config.UCEC_FINAL_DIR)
    parser.add_argument("--save_dir",     type=str,   default=config.UCEC_CHECKPOINT_DIR)
