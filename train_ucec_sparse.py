"""
train_ucec_sparse.py
=====================
Training script cho MoXGATE (SPARSEMAX) — UCEC.

4 phân lớp: CN_LOW (0) / CN_HIGH (1) / MSI (2) / POLE (3)

Cách chạy:
    !python MoXGATE/train_ucec_sparse.py
"""
...
def plot_confusion_matrix(y_true, y_pred, labels, save_path):
    try:
        ...
        plt.title('Confusion Matrix - UCEC (Sparsemax)')
...
def train(args):
    ...
    print(f"\n{'★'*60}")
    print("  MoXGATE — TRAINING (UCEC — Sparsemax)")
    print(f"{'★'*60}")
    ...
    print(f"[Train UCEC Sparse] Trainable parameters: {model.count_parameters():,}\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            torch.save({"model_state": model.state_dict(), "config": model.config},
                       os.path.join(args.save_dir, "best_model_ucec_sparse.pt"))
            print(f"  ✓ Saved best model (val_acc={best_val_acc:.4f})")
        else:
            patience_counter += 1
            if patience_counter >= args.patience:
                print(f"\n[Train UCEC Sparse] Early stopping tại epoch {epoch}")
                break

    # ── Test ──
    checkpoint = torch.load(os.path.join(args.save_dir, "best_model_ucec_sparse.pt"),
                            map_location=device)
    ...
    print(f"\n{'─'*60}")
    print("  ĐÁNH GIÁ TEST SET (UCEC Sparsemax)")
    print(f"{'─'*60}")
    ...
    plot_confusion_matrix(test_targets, test_preds, present_names,
                          os.path.join(args.save_dir, "confusion_matrix_ucec_sparse.png"))

    with open(os.path.join(args.save_dir, "history_ucec_sparse.json"), "w") as f:
        json.dump(history, f, indent=2)

    print(f"\n{'★'*60}")
    print(f"  HOÀN TẤT UCEC Sparse — Val Acc: {best_val_acc:.4f} | Test Acc: {test_metrics['accuracy']:.4f} | Test F1: {test_metrics['weighted_f1']:.4f}")
    print(f"{'★'*60}")
    return test_metrics
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MoXGATE Sparse — UCEC")
